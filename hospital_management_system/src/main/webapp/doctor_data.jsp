        ...
        <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur"
          ...
          <div class="container-fluid py-1 px-3">
            <nav aria-label="breadcrumb">
              ...
              <h6 class="font-weight-bolder text-white mb-0">Doctor's Table</h6>
            </nav>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                  <div class="table-responsive p-0">
                    <table class="table align-items-center mb-0" id="dataTable" style="margin-left: 300px; width: 70%;" >
                      ...
                      <tbody>
                        <c:forEach var="doctor" items="${doctord}">
                          <tr>
                            <td>
                            <label class="s" for="" id="lblsl_no"> ${doctor.sl_no}</label>

                          </td>

                            <td>
                              <label class="s" for="" id="lblname">${doctor.name}</label>
                              <input class="h" type="text" id="txtname" value="${doctor.name}">
                            </td>

                            <td>
                              <label class="s" for="" id="lblemail">${doctor.email}</label>
                              <input class="h" type="text" id="txtemail" value="${doctor.email}" >
                            </td>

                            <td>
                              <label class="s" for="" id="lblrole">${doctor.role}</label>
                              <input class="h" type="text" id="txtrole" value="${doctor.role}" >
                            </td>

                            <td>
                              <label class="s" for="" id="lblstatus">${doctor.status}</label>
                              <input class="h" type="text" id="txtstatus" value="${doctor.status}">
                            </td>
      ...
      <script>
        document.getElementById('searchInput').addEventListener('input', function () {
            ...
            for (let i = 1; i < rows.length; i++) {  // Start from 1 to skip header row
               ...
               // let password = rows[i].getElementsByTagName('td')[3].textContent.toLowerCase();
                let role = rows[i].getElementsByTagName('td')[4].textContent.toLowerCase();
                let status = rows[i].getElementsByTagName('td')[5].textContent.toLowerCase();
    
                if (name.includes(searchValue) || email.includes(searchValue) || role.includes(searchValue) || status.includes(searchValue)) {
    ...
    <script>
      $(document).ready(function(){
        $("input[type='button']").click(function(){
          ...
          var role=tr.find('#txtrole').val();
          var status=tr.find('#txtstatus').val();
         if(btn=="Delete"){
          $.ajax({
            url:"/delete_doctor",
            type:"post",
          ...
          tr.find('#lblstatus').addClass('h').removeClass('s');
          tr.find('#txtrole').addClass('s').removeClass('h');
          tr.find('#txtstatus').addClass('s').removeClass('h');
